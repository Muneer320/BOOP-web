...
const FileUploader = ({ label, accept, onFileUploaded, description, hasDefaultOption = false, defaultFile = '' }) => {
    ...
    <div className={`file-uploader ${isUploaded ? 'uploaded' : ''}`}>
      ...
      {hasDefaultOption && (
        <div className="default-option">
          <label className="toggle-container">
            ...
            />
            <span className="toggle-label">Use default {label.toLowerCase()}</span>
          </label>
        </div>
      )}
      ...
      <div className={`upload-area ${useDefault ? 'disabled' : ''}`}>
        ...
        <label 
          ...
          className={`file-label ${isUploading ? 'uploading' : ''} ${useDefault ? 'disabled' : ''}`}
        >
          {isUploading ? 'Uploading...' :
           isUploaded ?
           <span className="uploaded-text">✓ {fileName}</span> :
           useDefault ? `Using ${defaultFile}` : 'Choose File'}
        </label>
      </div>
